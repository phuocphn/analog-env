...
def synthesis_level2():
    ...
    for idx, variant_name in enumerate(["Pmos", "Nmos"]):

        return_circuits = [
            getattr(differentialpairs, "getDifferentialPair" + variant_name)(),
        ]
        ...
        # write to file
        with open(
            f"library/HL2/dp/{idx}-{variant_name}-{len(return_circuits)}.log", "w"
        ) as fw:
            fw.write("\n--------------------------")
