...
def synthesis_level2():
    ...
    for idx, variant_name in enumerate(
        ["getAllCurrentBiasesPmos", "getAllCurrentBiasesNmos"]
    ):

        return_circuits = getattr(currentbiases, variant_name)()
        ...
        # write to file
        with open(
            f"library/HL2/cp/{idx}-{variant_name}-{len(return_circuits)}.log", "w"
        ) as fw:
            fw.write("\n--------------------------")
