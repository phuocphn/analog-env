...
def synthesis_level2():
    ...
    for idx, variant_name in enumerate(
        ...
        return_circuits = getattr(analoginverters, variant_name)()

        for circuit in return_circuits:
            print(get_circuit_tree(circuit, print_level=0))
            print("++++++++++++++++++++++++")
        print("****************************************************" * 2)

        # write to file
        with open(
            f"library/HL2/inv/{idx}-{variant_name}-{len(return_circuits)}.log", "w"
        ) as fw:
            fw.write("\n--------------------------")
